...
TAG_VERSION="${TAG#v}"
PLIST_VERSION=$(grep -m1 'CFBundleShortVersionString:' "$PROJECT_YML" | sed -E 's/.*CFBundleShortVersionString: *"([^"]+)".*/\1/')
...
if [ "$TAG_VERSION" != "$PLIST_VERSION" ]; then
  echo "error: tag version '$TAG_VERSION' does not match CFBundleShortVersionString '$PLIST_VERSION' in $PROJECT_YML" >&2
  exit 1
fi
