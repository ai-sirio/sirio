...
| File-system watch | `tiller_markdown/src/file_events.rs` | real `inotify` | Windows: real `ReadDirectoryChangesW` (overlapped, non-blocking `poll()`); macOS/other: `Err(Unsupported)` — FSEvents/kqueue remain a legitimate port target |
