...
| **there is no attach mode** — a second invocation restarts everything | every invocation kills and relaunches the compositor and the app, including under `TILLER_WL_KEEP=1`. So a gesture chain cannot be split across invocations: the app you drive in call 2 is a *different process* from the one call 1 left running, with the context menu closed, the sheet dismissed and any modal gone. Worse for portal work — the new sway picks a fresh `wayland-N`, orphaning a portal fix bound to the old one. Two independent passes hit this on 2026-08-20: one lost a context menu between calls and had to fold the whole sequence into a single action string; the other, unable to do that for a `TILLER_WL_KEEP` portal session, wrote a throwaway driver copying `pointer_command`/`click`/`rightclick`/`shot` **verbatim** out of this script and pointing them at the live session's existing FIFO and socket | **not fixed — known limitation, and the workaround has a cost worth weighing.** Default to putting the entire gesture chain in **one** action string; that is the supported shape and it is usually enough. Only reach for a private driver when you must attach to an already-running `KEEP` session, and understand that copied primitives are a fork of harness internals that will drift silently when the originals change (the `shot`-forces-a-real-resize and `pgrep`-truncation traps in this table are exactly the kind of knowledge a copy loses). A real `TILLER_WL_ATTACH=1` that reuses the existing compositor, socket and input FIFO instead of re-booting is the right fix and is **not** built |
