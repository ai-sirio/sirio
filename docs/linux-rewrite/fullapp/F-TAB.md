...
| F-TAB-09 | **SUPERSEDED — now PASSED; the verdict below is correct only for the recipe it used** | **Read this first (added 2026-08-20).** A later pass (`934ed922`, wave P) drove the real native GTK Open File dialog end to end **on this same host** — nested sway, Italian-locale GNOME chooser, the dialog confirmed as a real sway tile via `swaymsg -t get_tree` — and opened both a markdown file and a code file into the correct editor modes. The ledger records F-TAB-09 as PASSED on that evidence. The UNREACHABLE finding below is **not** wrong about what it did; it is wrong as a statement about the app, because it stops one layer short of the working recipe. Two layers are needed and it applied neither: (1) `dbus-update-activation-environment` with the compositor's real `$WAYLAND_DISPLAY` **before the first portal-triggering click** — ordering is the whole trick; and (2) Tiller's own startup bus-activates a portal process *before* step 1 can matter, and if that process's GTK backend has already died it holds `org.freedesktop.portal.Desktop` while marking the interface unavailable for its whole lifetime. Find it with `dbus-send … GetNameOwner` + `GetConnectionUnixProcessID`, kill it, confirm `NameHasNoOwner`, then start a fresh portal. `TILLER_WL_PORTAL=1` and `GDK_BACKEND=wayland` do neither, which is why both attempts below failed identically — a stale portal is indistinguishable from an absent one at the app's error toast. Kept in full rather than deleted: it is the accurate record of what the lesser recipe produces, and "cannot open display" is what you will see again if you skip step 2. | `ctl` + `chord ctrl o` reliably reaches the app's Open File action, but the native GTK file picker fails every time with the app's own clean error toast: *"[Files] could not open the file picker: Couldn't open file picker due to missing xdg-desktop-portal implementation."* Root-caused via `/tmp/ftabfile-dbus.log`: `xdg-desktop-portal-gtk` itself fails with `Gtk-WARNING: cannot open display` because this sway session is pure-Wayland (no X11 fallback for the portal backend). Tried both `TILLER_WL_PORTAL=1` alone (`ftabfile-out`) and with `GDK_BACKEND=wayland` added (`ftabfile2-out`) — same failure both times. This blocks F-TAB-01's document/diff-tab sub-case too. Not a defect: the app surfaces the missing-portal condition correctly rather than hanging or crashing. |
