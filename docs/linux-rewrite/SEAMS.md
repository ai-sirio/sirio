...
| **Project icon persistence** | ✅ `ProjectRecord` has `icon_kind`/`icon_value`, but the app catalog model currently drops the picker value. | 🔴 **`codex11`** — carry `ProjectIcon` through the persistence/domain boundary and add the durable update/reload path; do not make the sidebar reach into `tiller_persistence`. | `F-PER-07` |
