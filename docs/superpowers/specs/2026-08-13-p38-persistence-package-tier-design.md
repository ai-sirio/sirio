...
The package will install a persistent SQLite `max_page_count` derived from a 64 MiB logical database limit. Per-pane scrollback remains bounded at 256 KiB in the session layer; the package-level cap prevents the SQLite file itself from growing without limit. `PersistenceError` will expose a dedicated oversized-store error alongside corruption and newer-schema errors.
