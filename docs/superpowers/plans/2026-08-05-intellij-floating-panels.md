...
- **Never verify an app-target test with `-only-testing:TillerTests/<functionName>`.** These are swift-testing free functions, not XCTest methods; the selector matches nothing, xcodebuild runs zero tests, and still prints `** TEST SUCCEEDED **`. A green run that proves nothing is worse than a red one. Always run the whole app suite and confirm the test by name plus a non-zero count:
...
xcodebuild test -project Tiller.xcodeproj -scheme Tiller -configuration Debug \
  ...
  -clonedSourcePackagesDirPath DerivedData/SourcePackages > /tmp/apptests.log 2>&1
grep -E "<theTestFunctionName>" /tmp/apptests.log
grep -E "Test run with [1-9][0-9]* tests" /tmp/apptests.log
```

- `Tiller.xcodeproj/project.pbxproj` is generated by `xcodegen generate` but **is tracked in git** and must be committed whenever files are added or removed (see the repo's own `chore: register … sources` commits). `CLAUDE.md` calls it gitignored; that is stale — `.gitignore` lists it, but the file was tracked before the rule existed, so the rule never applied.
- Two SwiftLint build commands fail for `CodeEditTextView` and `CodeEditSourceEditor` because `swiftlint` is not installed on this machine. Pre-existing environment noise, unrelated to this work; do not try to fix it.
...
xcodebuild test -project Tiller.xcodeproj -scheme Tiller -configuration Debug \
  -skipPackagePluginValidation -skipMacroValidation -skipPackageUpdates \
  -clonedSourcePackagesDirPath DerivedData/SourcePackages 2>&1 | tail -20
```
...
xcodebuild test -project Tiller.xcodeproj -scheme Tiller -configuration Debug \
  -skipPackagePluginValidation -skipMacroValidation -skipPackageUpdates \
  -clonedSourcePackagesDirPath DerivedData/SourcePackages > /tmp/apptests.log 2>&1
grep -E "dividerCentreLandsInTheMiddleOfTheVisibleGap|dividerCentreIsTheColumnEdgeWhenThereIsNoGap" /tmp/apptests.log
grep -E "Test run with [1-9][0-9]* tests" /tmp/apptests.log
```
Expected: both test names appear with `✔ … passed`, and the run reports a non-zero test count.
