...
/// Mirrors the real, enforced split-pane minimum applied by `sirio`'s own
/// pane-tree layout (`MIN_SPLIT_PANE_SIZE` in `sirio/src/main.rs`, applied
/// via `.min_w()/.min_h()` on every split child). `sirio_terminal` cannot
/// import that constant directly -- `sirio` depends on `sirio_terminal`,
/// not the reverse -- so the value is intentionally duplicated here rather
/// than reused from `sirio::panes::split_disabled_reason`'s own, different,
...
/// above for why `sirio_terminal` cannot import `sirio::panes` directly)
