...
//!
//! `sirioctl` exposes no `browser` command, so #142 cannot be exercised from
//! it. This uses the same `client::round_trip` transport `sirioctl` does,
...
use sirio_control::client;
use sirio_control::protocol::ControlRequest;
