//! The blocking control client used by `sirioctl` and by tests. Ported from
//! `SirioControl/ControlClient.swift`: a fresh connection per request, one
...
pub enum ClientError {
    ...
    /// Connecting failed (socket missing, Sirio not running).
...
///
/// Blocking by design — `sirioctl` is a short-lived CLI. `timeout` bounds
