...
pub(crate) fn pipe_name_for_path(path: &Path) -> Result<String, String> {
    ...
    // releases (a changed hash would split the app and sirioctl onto
            ...
            Err(error) => {
                ...
                if error.raw_os_error() == Some(ERROR_ACCESS_DENIED) {
                    match open_client(path) {
                        ...
                        // live sibling Sirio — or a same-user squatter,
...
pub(crate) fn open_client(path: &Path) -> Result<PipeStream, ClientConnectError> {
    ...
    // error instead of hanging sirioctl.
...
mod tests {
    ...
    /// whatever sirioctl writes; a DACL only says who may CONNECT to a pipe
