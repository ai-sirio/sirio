...
fn main() {
    #[cfg(target_os = "windows")]
    {
        ...
        // `sirio_terminal/build.rs` puts it on the link line. Both therefore
        ...
        // explicitly; and emitting either flag from `sirio_terminal` is inert
        ...
        let rc = std::path::Path::new(&std::env::var("OUT_DIR").expect("OUT_DIR")).join("sirio.rc");
        ...
        std::fs::write(
            &rc,
            format!("1 ICON \"{icon_literal}\"\n2 ICON \"{attention_icon_literal}\"\n"),
        )
        .expect("write sirio.rc");
