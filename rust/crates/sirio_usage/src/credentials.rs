...
/// fallback — `local_app_data` under `Sirio/credentials.json` on Windows,
/// `home` under `.local/share/sirio/credentials.json` on POSIX. On Windows
...
fn resolve_store_path(
    ...
    if let Some(data) = xdg_data_home.filter(|data| !data.is_empty()) {
        return Ok(Path::new(data).join("sirio/credentials.json"));
    }
    #[cfg(windows)]
    {
        if let Some(root) = local_app_data.filter(|root| !root.is_empty()) {
            return Ok(Path::new(root).join("Sirio").join("credentials.json"));
        }
    }
    #[cfg(not(windows))]
    {
        if let Some(home) = home.filter(|home| !home.is_empty()) {
            return Ok(Path::new(home).join(".local/share/sirio/credentials.json"));
        }
    }
...
mod tests {
    ...
    impl TempStoreDir {
        fn new(label: &str) -> Self {
            let path = std::env::temp_dir()
                .join(format!("sirio-credentials-{label}-{}", std::process::id()));
    ...
    fn store_path_precedence_is_explicit_then_xdg_then_home() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            PathBuf::from("/xdg/sirio/credentials.json")
        );
        #[cfg(not(windows))]
        {
            assert_eq!(
        ...
        #[cfg(windows)]
        {
            assert_eq!(
                ...
                PathBuf::from(r"C:\Users\u\AppData\Local\Sirio\credentials.json"),
                "an empty override is no override"
            );
        }
        assert!(matches!(
    ...
    fn the_store_resolves_under_local_app_data_on_windows() {
        ...
        assert!(
            path.ends_with(r"Sirio\credentials.json"),
            "the store is Sirio\\credentials.json under that root: {}",
            path.display()
        );
    }
