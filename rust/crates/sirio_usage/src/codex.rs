...
/// `codex` itself may store other fields Sirio doesn't know about.
...
/// the actual `sirio` binary, not just a unit test) only by pointing this
...
mod tests {
    ...
    /// the auth file's fields Sirio doesn't know about survive the merge.
    #[test]
    fn a_real_refresh_success_merges_into_the_auth_file_without_losing_unrelated_fields() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "sirio-codex-real-refresh-merge-{}",
            std::process::id()
        ));
    ...
    /// `sirio_ui/src/status_bar.rs` calls (`executor.spawn(async move {
    ...
    fn a_real_200_response_completes_a_full_fetch_through_codexusagefetcher_fetch() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "sirio-codex-usg07-usage-success-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
    ...
    fn saving_writes_a_parseable_last_refresh_and_keeps_unknown_token_fields() {
        let dir =
            std::env::temp_dir().join(format!("sirio-codex-last-refresh-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("auth.json");
        std::fs::write(
            &path,
