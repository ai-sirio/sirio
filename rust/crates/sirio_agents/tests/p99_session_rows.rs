...
/// three hook events (one stale sirioctl, one current sirioctl, one
    ...
    serde_json::json!({
        ...
        "hooks": {
            "SessionStart": [{"matcher": "", "hooks": [{
                "type": "command",
                "command": format!(
                    "'/stale/DerivedData/sirioctl' notify --session {PANE} --status running --stdin-json"
                ),
                "timeout": 5
            }]}],
            "Stop": [{"matcher": "", "hooks": [{
            ...
            "PreToolUse": [{"matcher": "Bash", "hooks": [{
                "type": "command",
                "command": "/usr/bin/logger sirio-pre"
            }]}]
        },
...
fn migration_leaves_matching_malformed_and_unrelated_files_byte_identical() {
    ...
    // Matching: the only sirioctl command already carries the current path.
    ...
    // Unrelated: hooks that never mention sirioctl, plus other keys.
    ...
    for path in [&matching_path, &malformed_path, &unrelated_path] {
        ...
        assert!(
            !ClaudeHookMigrator::migrate_file(path, CURRENT_SIRIOCTL),
            "{} must be a no-op",
            path.display()
        );
        assert_eq!(
