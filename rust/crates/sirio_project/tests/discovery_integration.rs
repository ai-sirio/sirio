...
//! parse pipeline, exactly as the Swift app's `SirioGitTests` do.
...
fn make_git_repo() -> TempDir {
    ...
    git(dir.path(), &["config", "user.email", "test@sirio.dev"]);
...
fn load_project_wires_a_real_repo_into_the_workspace() {
    ...
    let tab_id = workspace
        .add_tab(worktrees[0].id, "Chat", sirio_project::TabKind::AgentChat)
