...
impl TempDir {
    fn new(tag: &str) -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("tiller-git-p41-{tag}-{}-{id}", std::process::id()));
...
fn directory_status_aggregates_ancestors_with_precedence_and_renames() {
    ...
    let snapshot = tiller_git::status(repository.path()).expect("status after merge");
...
fn side_by_side_preserves_hunks_pairs_runs_and_drops_metadata() {
    let diff = tiller_git::parse_diff(
        concat!(
...
fn side_by_side_handles_real_rename_binary_and_large_context() {
    ...
    let snapshot = tiller_git::status(repository.path()).expect("real status");
    let rename = snapshot
        .entries
        .iter()
    ...
    let diff = tiller_git::diff_entry(
        repository.path(),
        rename,
        tiller_git::WHOLE_FILE_CONTEXT_LINES,
    )
    ...
    let binary_entry = snapshot
        ...
        .expect("binary entry");
    assert!(
        tiller_git::diff_entry(repository.path(), binary_entry, 3)
...
fn mutation_validation_rejects_stale_and_duplicate_entries() {
    ...
    let stale = tiller_git::status(repository.path()).unwrap().entries;
    ...
    assert!(matches!(
        tiller_git::GitActions::discard_untracked(repository.path(), &[stale_entry]),
        Err(GitError::Action(_))
    ));

    std::fs::write(repository.path().join("duplicate.txt"), "duplicate\n")
        .expect("write duplicate fixture");
    let entry = tiller_git::status(repository.path())
        .unwrap()
        .entries
        .into_iter()
        ...
        .unwrap();
    assert!(matches!(
        tiller_git::GitActions::stage(repository.path(), &[entry.clone(), entry]),
        Err(GitError::Action(_))
    ));
}
