//! Linux display-backend policy: Tiller asks for X11.
//!
//! # Why
//!
//! The Browser tab's page lives in a real X11 child window reparented into
//! Tiller's own window — that is what `wry`'s `build_as_child` does on Linux.
...
//! session Tiller runs as an XWayland client and the browser works everywhere.
