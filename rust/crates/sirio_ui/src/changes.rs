...
mod tests {
    ...
    /// refresh, and a later refresh must clear it once the repository is
    /// usable again.
    #[gpui::test]
    async fn a_failed_stage_surfaces_the_git_error_and_recovers_after_refresh(
        cx: &mut TestAppContext,
    ) {
        let dir = TempDir::new();
        clean_git_repo(&dir.0);
        ...
        assert!(
            ...
            "the visible error keeps git's actionable lock detail: {error}"
        );
        ...
        pump_until(cx, || tab.read_with(cx, |tab, _| tab.git_error.is_none()));
    }
