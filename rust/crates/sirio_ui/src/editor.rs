...
use sirio_markdown::{DocumentChange, DocumentError, MarkdownDocument};
...
pub mod fs_actions {
    ...
    pub fn open_command(target: &OsStr) -> Command {
        ...
        {
            let mut command = Command::new("__sirio_unsupported_platform_open__");
            command.arg(target);
            command
        }
    }
    ...
    pub fn copy_path_text(path: &Path) -> String {
        if path.is_absolute() {
            sirio_project::display_absolute_path(path)
        } else {
            std::env::current_dir()
                .map(|dir| sirio_project::display_absolute_path(&dir.join(path)))
                .unwrap_or_else(|_| sirio_project::display_absolute_path(path))
        }
    }
}
...
mod tests {
    ...
    impl TempFile {
        ...
        fn with_suffix(name: &str, extension: &str, contents: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "sirio-editor-{name}-{}-{id}{extension}",
                std::process::id()
            ));
    ...
    fn save_that_cannot_recreate_shows_a_visible_error() {
        ...
        let doomed =
            std::env::temp_dir().join(format!("sirio-editor-doomed-{}", std::process::id()));
    ...
    fn registry_keys_are_canonical_so_aliased_paths_dedupe() {
        ...
        let alias = real
            .parent()
            .expect("parent")
            .join(format!("sirio-editor-alias-{}", std::process::id()));
    ...
    fn a_missing_path_and_an_unreadable_path_have_distinct_messages() {
        let missing_path = std::env::temp_dir().join(format!(
            "sirio-editor-missing-{}-never-written",
            std::process::id()
        ));
