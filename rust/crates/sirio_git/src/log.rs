...
mod tests {
    ...
    fn pathspecs_are_kept_apart_from_the_predicate_arguments() {
        let filter = LogFilter {
            paths: vec![PathBuf::from("rust/crates/sirio_git")],
            ..LogFilter::default()
        };
        ...
        assert_eq!(
            ...
            vec!["--".to_owned(), "rust/crates/sirio_git".to_owned()]
        );
    }
    ...
    fn a_text_filter_reaches_git_and_still_paginates() {
        let dir = std::env::temp_dir().join(format!("sirio-log-filter-{}", std::process::id()));
    ...
    fn a_body_is_fetched_for_one_commit_at_a_time() {
        let dir = std::env::temp_dir().join(format!("sirio-log-body-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("create temp repo");
