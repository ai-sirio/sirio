...
impl TempDir {
    fn new() -> Self {
        ...
        let path =
            std::env::temp_dir().join(format!("sirio-git-test-{}-{unique}", std::process::id()));
...
fn entry_for<'a>(
    snapshot: &'a sirio_git::StatusSnapshot,
    path: &str,
) -> &'a sirio_git::StatusEntry {
...
fn status_and_diff_handle_spaces_and_non_ascii_paths() {
    ...
    assert_eq!(
        ...
        Some(sirio_git::StatusKind::Modified)
    );
    ...
    let diff =
        diff_entry(repo.path(), spaced, sirio_git::DEFAULT_CONTEXT_LINES).expect("diff runs");
...
fn rename_reports_old_and_new_path() {
    ...
    assert_eq!(rename.index_status, Some(sirio_git::StatusKind::Renamed));
    assert_eq!(
    ...
    let diff =
        diff_entry(repo.path(), rename, sirio_git::DEFAULT_CONTEXT_LINES).expect("diff runs");
...
fn staged_and_worktree_modified_reports_two_states() {
    ...
    assert_eq!(entry.index_status, Some(sirio_git::StatusKind::Modified));
    assert_eq!(entry.worktree_status, Some(sirio_git::StatusKind::Modified));
    ...
    let diff = diff_entry(repo.path(), entry, sirio_git::DEFAULT_CONTEXT_LINES).expect("diff runs");
...
fn untracked_file_has_no_head_diff() {
    ...
    let diff =
        diff_entry(repo.path(), untracked, sirio_git::DEFAULT_CONTEXT_LINES).expect("diff runs");
...
fn binary_file_reports_no_line_counts() {
    ...
    let diff =
        diff_entry(repo.path(), binary, sirio_git::DEFAULT_CONTEXT_LINES).expect("diff runs");
...
fn unborn_head_reports_staged_and_untracked_and_diffs_against_dev_null() {
    ...
    assert_eq!(staged.index_status, Some(sirio_git::StatusKind::Added));
    ...
    let diff =
        diff_entry(repo.path(), staged, sirio_git::DEFAULT_CONTEXT_LINES).expect("diff runs");
    assert_eq!(diff.additions, 1);
...
fn conflicted_file_is_reported_as_unmerged() {
    ...
    assert_eq!(entry.index_status, Some(sirio_git::StatusKind::Unmerged));
}
...
fn stage_refuses_a_conflicted_path_without_changing_git() {
    ...
    assert_eq!(
        error,
        sirio_git::GitError::ConflictedPaths {
