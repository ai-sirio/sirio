...
use sirio_git::{GitLog, LogFilter};
...
fn commit_files_and_diff_support_merge_commits() {
    ...
    let files = sirio_git::commit_files(dir.path(), &sha).expect("files");
    let diff = sirio_git::commit_diff_entry(dir.path(), &sha, Path::new("b.txt")).expect("diff");
...
fn commit_files_and_diff_preserve_utf8_paths() {
    ...
    let files = sirio_git::commit_files(dir.path(), &sha).expect("files");
    let diff = sirio_git::commit_diff_entry(dir.path(), &sha, path).expect("diff");
...
fn commit_diff_entry_ignores_forced_git_colors() {
    ...
    let diff = sirio_git::commit_diff_entry(dir.path(), &sha, Path::new("café.txt")).expect("diff");
