...
//! `TillerControl/ControlServer.swift`.
//!
...
//!   bind never fails forever; a *live* socket (another running Tiller) is
//!   never stolen.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ServerError::AlreadyRunning { path } => {
                write!(f, "a Tiller is already listening at {}", path.display())
            }
