...
//! question and nothing here should survive into `tiller_terminal`.
//!
...
//!
//! ```text
//! cargo run -p tiller_terminal --example prototype_ghostty_pane
//! ```
                ...
                div()
                    ...
                    .children(line.into_iter().map(|run| {
                        let (fg, bg) = if run.inverse {
                            (run.bg.unwrap_or((16, 16, 20)), run.fg.unwrap_or((208, 208, 216)))
                        } else {
                            (
...
//
// Run: cargo test -p tiller_terminal --example prototype_ghostty_pane \
//          -- --test-threads=1
// ===========================================================================
#[cfg(test)]
mod tests {
    ...
    const SENTINEL: &str = "TILLERPROTOOK7391";
    ...
    /// handler wired (`on_title_changed` et al.)? Tiller's activity Layer B
    ...
    fn osc_title_reaches_the_terminal_state() {
        ...
        pane.feed_vt(b"\x1b]2;TILLERPROTO-TITLE\x07");

        let title = pane.title();
        assert!(
            title.contains("TILLERPROTO-TITLE"),
    ...
    fn probe_dsr(argv: &[&str], patience: Duration) -> (bool, bool, String) {
        ...
        loop {
            ...
            match rx.recv_timeout(left) {
                Ok(bytes) => {
                    ...
                    total.extend_from_slice(&bytes);
                    if total.windows(SENTINEL.len()).any(|w| w == SENTINEL.as_bytes()) {
                        break;
                    }
                }
        ...
        // positive on the tiller form, where cmd.exe ECHOES the mangled
    ...
    fn windows_shell_invocation_survives_command_builder() {
        // Form 1 — exactly what tiller_terminal::command_shell_invocation
        // produces: `/C` plus the command pre-wrapped in quotes.
        let (tiller_exec, tiller_mangled, tiller) = probe_dsr(
            &["cmd.exe", "/C", "\"echo TILLERPROTOOK7391\""],
        ...
        println!("TILLER FORM (pre-quoted): {tiller}");
        println!("PLAIN FORM:               {plain}");

        // Document reality: plain wins, pre-quoted loses, and the tiller
        // form's capture carries the escaped-quote mangling that explains it.
        assert!(plain_exec, "plain form did not execute: {plain}");
        assert!(!tiller_exec, "prediction WRONG — pre-quoted form executed too: {tiller}");
        assert!(
            tiller_mangled,
    ...
    fn pty_alone_delivers_bytes() {
        let report = [
            ...
            probe(
                &["powershell.exe", "-NoProfile", "-Command", "Write-Output 42"],
                Duration::from_secs(10),
            ),
