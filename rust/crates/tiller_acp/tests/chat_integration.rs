...
use tiller_acp::{AcpError, AgentCommand, ChatSession, ChatSessionConfig, ChatStatus};
...
fn wait_for<F>(session: &ChatSession, mut predicate: F) -> tiller_acp::ChatSnapshot
where
    F: FnMut(&tiller_acp::ChatSnapshot) -> bool,
{
...
fn chat_session_streams_tool_permission_stops_and_restores() {
    ...
    let streaming = wait_for(&session, |snapshot| {
        snapshot.status == ChatStatus::Streaming
            && snapshot
                ...
                .any(|entry| matches!(entry, tiller_persistence::ChatEntry::AssistantMessage { text } if text == "first "))
    });
            ...
            turn.entries.iter().any(|entry| {
                matches!(
                    ...
                    tiller_persistence::ChatEntry::Permission {
                        request_id: 1,
                        outcome: tiller_persistence::ChatPermissionOutcome::Pending,
                        ..
                    }
                )
            })
    ...
    assert!(pending.transcript.turns[0].entries.iter().any(|entry| {
        matches!(entry, tiller_persistence::ChatEntry::ToolCall { id, .. } if id == "tool-1")
    }));
    session
    ...
    assert!(completed.transcript.turns.iter().any(|turn| {
        turn.entries.iter().any(|entry| {
            matches!(
                entry,
                tiller_persistence::ChatEntry::Permission {
                    outcome: tiller_persistence::ChatPermissionOutcome::Selected { option_id, .. },
                    ..
                } if option_id == "deny"
            )
        })
    ...
    assert!(completed.transcript.turns.iter().any(|turn| {
        turn.entries.iter().any(|entry| {
            matches!(entry, tiller_persistence::ChatEntry::ToolCall { status, .. } if status == "Completed")
        })
    }));
    ...
    wait_for(&cancellable, |snapshot| {
        snapshot.status == ChatStatus::Streaming
            && snapshot.transcript.turns.iter().any(|turn| {
                turn.entries.iter().any(|entry| {
                    matches!(entry, tiller_persistence::ChatEntry::AssistantMessage { text } if text == "partial")
                })
            })
    });
...
fn chat_session_sends_queued_text_after_a_normal_turn() {
    ...
    assert!(matches!(
        ...
        tiller_persistence::ChatEntry::UserMessage { text } if text == "first prompt"
    ));
    assert!(matches!(
        &completed.transcript.turns[1].entries[0],
        tiller_persistence::ChatEntry::UserMessage { text } if text == "second prompt"
    ));
...
fn chat_session_folds_plan_approval_and_persists_it() {
    ...
    let snapshot = wait_for(&session, |snapshot| {
        snapshot.status == ChatStatus::Completed
            && snapshot.transcript.turns.iter().any(|turn| {
                turn.entries
                    .iter()
                    .any(|entry| matches!(entry, tiller_persistence::ChatEntry::Plan { .. }))
            })
    });
    let plan = snapshot
        .transcript
        ...
        .find_map(|entry| match entry {
            tiller_persistence::ChatEntry::Plan { entries } => Some(entries),
            _ => None,
        })
    ...
    let permission = snapshot
        ...
        .flat_map(|turn| turn.entries.iter())
        .find_map(|entry| match entry {
            tiller_persistence::ChatEntry::Permission { title, outcome, .. } => {
                Some((title, outcome))
            }
            _ => None,
        })
...
fn chat_session_answers_a_question_with_text_and_can_cancel_one() {
    ...
    let answered = snapshot
        ...
        .flat_map(|turn| turn.entries.iter())
        .find_map(|entry| match entry {
            tiller_persistence::ChatEntry::Permission { outcome, .. } => Some(outcome),
            _ => None,
        })
        .expect("the answered card was persisted");
    assert!(matches!(
        answered,
        tiller_persistence::ChatPermissionOutcome::Selected { option_id, label }
...
fn chat_session_expires_a_permission_left_open_by_a_dead_transport() {
    ...
    assert!(snapshot.transcript.turns.iter().any(|turn| {
        turn.entries
            .iter()
            .any(|entry| matches!(entry, tiller_persistence::ChatEntry::Error { .. }))
    }));
