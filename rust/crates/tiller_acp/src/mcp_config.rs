...
mod tests {
    ...
    fn missing_file_yields_no_servers() {
        let dir = std::env::temp_dir().join("tiller-mcp-config-test-missing");
    ...
    fn malformed_json_yields_no_servers_not_a_panic() {
        let dir = std::env::temp_dir().join("tiller-mcp-config-test-malformed");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn a_stdio_server_pointing_at_a_nonexistent_binary_still_parses() {
        ...
        let dir = std::env::temp_dir().join("tiller-mcp-config-test-broken-stdio");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join(".mcp.json"),
    ...
    fn args_and_env_carry_through() {
        let dir = std::env::temp_dir().join("tiller-mcp-config-test-args-env");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join(".mcp.json"),
    ...
    fn a_remote_http_server_parses() {
        let dir = std::env::temp_dir().join("tiller-mcp-config-test-http");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join(".mcp.json"),
    ...
    fn an_entry_with_neither_command_nor_url_is_skipped_not_panicked() {
        let dir = std::env::temp_dir().join("tiller-mcp-config-test-empty-entry");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join(".mcp.json"),
