...
pub struct DragState {
    ...
    // F-CORE-FILE-03A / P133: set when a `wl_data_device::Event::Drop` arrives before the
    // async `Enter` pipe-read below has finished resolving `text/uri-list` into paths -- a
    // provider slower than roughly 150ms reliably wins that race. Previously `Drop` just
    // bailed out in that case (no `finish()`/`destroy()`, no event, no error logged), silently
    // discarding an already-committed drop with no way for the app to ever recover it, because
    // the resolution's own continuation had no way to know a `Drop` was waiting on it. This flag
    // lets that continuation finish the transfer itself once the paths are ready instead.
    pending_drop: PendingDrop,
}

/// State-transition wrapper around the `pending_drop` race flag, factored out of `DragState`
/// so it is unit-testable without a live Wayland connection -- see the `tests` module at the
/// bottom of this file. Mirrors exactly what the `Drop` and `Enter`-continuation match arms do
/// to this field. F-CORE-FILE-03A / P133.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub(crate) struct PendingDrop(bool);

impl PendingDrop {
    /// `wl_data_device::Event::Drop` arrived while `DragState::window` is still unset (the
    /// async `Enter` resolution has not completed yet). Flag it instead of losing the drop.
    fn mark(&mut self) {
        self.0 = true;
    }

    /// The `Enter` resolution has a result now, successful or not -- consume the flag either
    /// way (`Cell`-style `take`, not just a read), so a *later*, unrelated drag can never
    /// inherit a stale `true` left behind by this one, e.g. from its error/empty-list path.
    /// Returns whether a `Drop` was actually waiting on this resolution.
    fn take(&mut self) -> bool {
        std::mem::replace(&mut self.0, false)
    }
}

/// Where a `Drop` that raced the async `Enter` resolution should land its synthetic `Submit`.
///
/// Deliberately `drag.position`, not `Enter`'s own entry-point coordinate: `Motion` now tracks
/// `drag.position` unconditionally, even before `DragState::window` is set (see the `Motion`
/// match arm above), so by the time a slow provider's resolution finally completes this is
/// wherever the pointer actually was, not wherever it first crossed into the window. Using the
/// entry-point coordinate here was a real bug caught live while fixing this race
/// (F-CORE-FILE-03A / P133): the drop stopped being lost, but landed on whatever element sat
/// under the pointer's *entry* point instead of the real drop target, silently misdirected
/// rather than silently discarded. Extracted so the decision is unit-tested directly (see
/// `tests::a_pending_drop_submits_at_the_latest_tracked_position_not_the_entry_point`), rather
/// than only exercisable through a live compositor.
fn pending_drop_submit_position(drag: &DragState) -> Point<Pixels> {
    drag.position
}
    ...
    pub(crate) fn new() -> Self {
        ...
        let state = Rc::new(RefCell::new(WaylandClientState {
            ...
            drag: DragState {
                ...
                pending_drop: PendingDrop::default(),
            },
            external_drag: None,
            click: ClickState {
        ...
        match event {
            ...
            wl_data_device::Event::Enter {
                ...
                if let Some(data_offer) = data_offer {
                    ...
                    // F-CORE-FILE-03A / P133: track the latest known pointer position from the
                    // moment `Enter` arrives, not only once `state.drag.window` is set (which
                    // happens later, when the async resolution below completes). Otherwise a
                    // `Drop` that races that resolution (see `pending_drop`) would fall back to
                    // this stale entry-point coordinate instead of wherever the pointer actually
                    // was when the drop happened, and the synthetic click the fallback sends
                    // would land on whatever element sits under (x, y) at *entry*, not under the
                    // real drop target.
                        ...
                        .spawn(async move {
                            let file_list = match read_task.await {
                                ...
                                Err(err) => {
                                    ...
                                    // F-CORE-FILE-03A / P133: clear any `pending_drop` this
                                    // Enter's own eventual `Drop` set -- otherwise it would
                                    // wrongly be consumed by the *next* drag's resolution.
                                    this.get_client().borrow_mut().drag.pending_drop.take();
                                    return;
                                }
                            };
                            ...
                            // F-CORE-FILE-03A / P133: the compositor already delivered `Drop`
                            // while this read was still in flight (a slow provider outpacing
                            // it). There will be no second `Drop` event to consume this result,
                            // so finish the transfer here instead of leaving the offer to rot --
                            // see the `pending_drop` doc comment on `DragState` above.
                            if state.drag.pending_drop.take() {
                                let drop_position = pending_drop_submit_position(&state.drag);
            ...
            wl_data_device::Event::Motion { x, y, .. } => {
                // F-CORE-FILE-03A / P133: keep this ahead of the `window.is_some()` check (see
                // the comment on `Enter` above) so a `Drop` racing the async resolution still has
                // an accurate last-known position to fall back to.
                let position = Point::new(x.into(), y.into());
                state.drag.position = position;
            ...
            wl_data_device::Event::Drop => {
                let Some(drag_window) = state.drag.window.clone() else {
                    // F-CORE-FILE-03A / P133: the async `Enter` pipe-read (above) has not
                    // resolved `text/uri-list` into paths yet -- a slow-resolving provider.
                    // Wayland's data-device protocol never sends a second `Drop`, so if we
                    // bail out here with nothing recorded, that resolution's continuation
                    // has no way to know a drop is waiting on it and the transfer is lost
                    // for good, silently, once it does complete. Flag it instead; the
                    // continuation checks `pending_drop` and finishes the transfer itself.
                    state.drag.pending_drop.mark();
                    return;
                };
                let data_offer = state.drag.data_offer.clone().unwrap();
                data_offer.finish();
...
mod tests {
    ...
    // F-CORE-FILE-03A / P133 -- regression coverage for the slow-provider XDND drop that used
    // to be silently discarded. These test the extracted pure logic directly, since driving the
    // real `wl_data_device` dispatch needs a live Wayland connection this crate cannot fake;
    // the live reproduction lives in `Scripts/wayland-drive.sh`'s `xdnd --delay-ms N` action
    // (see docs/linux-rewrite/WAYLAND-LANE.md's XDND section).

    #[test]
    fn a_drop_before_resolution_is_flagged_and_consumed_exactly_once() {
        let mut pending = PendingDrop::default();

        // No `Drop` raced this resolution: nothing to consume.
        assert!(!pending.take());

        // `Drop` arrives while `Enter`'s async read is still in flight.
        pending.mark();
        assert!(pending.take(), "a marked drop must be reported once");

        // Consuming it clears the flag -- a second, unrelated resolution (e.g. the next drag)
        // must never inherit a stale `true` left over from this one.
        assert!(
            !pending.take(),
            "pending_drop must not still read true after being taken"
        );
    }

    #[test]
    fn a_pending_drop_submits_at_the_latest_tracked_position_not_the_entry_point() {
        ...
        // `Enter` fires near wherever the drag crossed into the window.
        drag.position = point(px(6.0), px(6.0));
        let entry_point = drag.position;

        // The pointer keeps moving toward the real drop target while the async pipe-read is
        // still in flight -- `Motion` tracks `drag.position` unconditionally now, even before
        // `DragState::window` is set.
        drag.position = point(px(350.0), px(200.0));
        drag.position = point(px(700.0), px(400.0));
        let real_drop_target = drag.position;
        assert_ne!(
            entry_point, real_drop_target,
            "the test must exercise a pointer that actually moved after Enter"
        );

        // `Drop` arrives before the resolution has stored a window.
        assert!(drag.window.is_none());
        drag.pending_drop.mark();

        // The resolution's own continuation must land the synthetic `Submit` at the real drop
        // target, not the stale point the drag entered the window at.
        assert_eq!(pending_drop_submit_position(&drag), real_drop_target);
    }
}
