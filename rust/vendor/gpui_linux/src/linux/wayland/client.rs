                ...
                if let Some(data_offer) = data_offer {
                    ...
                    state
                        ...
                        .spawn(async move {
                            ...
                            // F-CORE-FILE-03A / P133: the compositor already delivered
                            // `Drop` while this read was still in flight (a slow provider
                            // outpacing it). There will be no second `Drop` event to
                            // consume this result, so finish the transfer here instead of
                            // leaving the offer to rot -- see the `pending_drop` doc
                            // comment on `DragState` above.
